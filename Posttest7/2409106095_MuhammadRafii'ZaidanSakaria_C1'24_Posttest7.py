...
# Fungsi Dengan Parameter (contoh: total harga dari keranjang)
def hitung_total_harga(keranjang):
    total = 0  # Variabel lokal
    for manga in keranjang:
        total += manga['harga']
    return total


# Prosedur: Menampilkan Keranjang
def tampilkan_keranjang(keranjang):
    if not keranjang:
        print("Keranjang Anda kosong.")
    else:
        print("Daftar di Keranjang:")
        for indeks, manga in enumerate(keranjang, start=1):
            print(f"{indeks}. {manga['judul']} - Rp{manga['harga']}")


# Prosedur: Menambahkan Manga ke Keranjang
def tambah_ke_keranjang(keranjang, pilihan_manga):
    if pilihan_manga in Manga_List:
        keranjang.append(Manga_List[pilihan_manga])
    ...
    if opsi == "1":
        ...
        if Username == "Admin" and Password == "Admin123":
            ...
            while True:
                ...
                if status == "1":
                    tampilkan_manga()  # Memanggil fungsi tanpa parameter

                elif status == "2":
                    tampilkan_manga()  # Fungsi tanpa parameter
                    try:
                        pilihan_manga = int(input("Masukkan nomor manga yang ingin dibeli ke keranjang: "))
                        tambah_ke_keranjang(akuns[Username]["Keranjang"], pilihan_manga)  # Prosedur dengan parameter
                    except ValueError:
                        print("Input tidak valid. Harap masukkan nomor yang benar.")

                elif status == "3":
                    tampilkan_keranjang(akuns[Username]["Keranjang"])  # Prosedur dengan parameter

                elif status == "4":
                    if not akuns[Username]["Keranjang"]:
                        print("Keranjang Anda kosong.")
                    else:
                        tampilkan_keranjang(akuns[Username]["Keranjang"])  # Memanggil prosedur
                        try:
                    ...
                    if not akuns[Username]["Keranjang"]:
                        ...
                        total_harga = hitung_total_harga(akuns[Username]["Keranjang"])  # Fungsi dengan parameter
                        tampilkan_keranjang(akuns[Username]["Keranjang"])  # Memanggil prosedur
